...
public class TopicDAO {
    ...
    public static boolean add(String uname, String title) {
        ...
        try {
            ...
            ps = con.prepareStatement("insert into topics values(topicid,?,createdate,?)");
    ...
    public static boolean deleteQuestion(String questionid_sequence) {
        Connection con = null;
        PreparedStatement ps = null;
        try {
            con = Database.getConnection();
            ps = con.prepareStatement("delete from questions where questionid_sequence = ?");
            ps.setString(1, questionid_sequence);
            ps.executeUpdate();
            return true;
        } catch (Exception ex) {
    ...
    public static boolean addQuestion(Question q) {
        ...
        try {
            con = Database.getConnection();
            ps = con.prepareStatement("insert into questions values(questionid_sequence,?,?,?,?,?)");
    ...
    public static ArrayList<Topic> getTopics() {
        try {
            ...
            while (rs.next()) {
                al.add(new Topic(rs.getString("topicid"), rs.getString("topictitle"), rs.getString("createdate"), rs.getString("uname")));
            }
            rs.close();
            return al;
        } catch (Exception e) {
    ...
    public static ArrayList<SurveyResult> getSurveyResults(String topicid) {
        try {
            ...
            while (questions.next()) {
                ...
                sr.setQuestion(questions.getString("question"));
                ...
                PreparedStatement psa = con.prepareStatement(
                        "select sum(decode(answer,1,1,0)) opt1count, sum(decode(answer,2,1,0)) opt2count, sum(decode(answer,3,1,0)) opt3count from answers_details where questionid_sequence = ?");
                psa.setString(1, questions.getString("questionid_sequence"));
    ...
    public static ArrayList<Question> getQuestions(String topicid) {
        try {
            Connection con = Database.getConnection();
            PreparedStatement ps = con.prepareStatement("select * from questions where topicid = ? order by questionid_sequence");
            ps.setString(1, topicid);
            ResultSet rs = ps.executeQuery();

            ArrayList<Question> al = new ArrayList<Question>();
            while (rs.next()) {
                al.add(new Question(rs.getString("questionid_sequence"), rs.getString("question"), rs.getString("opt1"), rs.getString("opt2"), rs.getString("opt3")));
            }
            rs.close();
            return al;
        } catch (Exception e) {
